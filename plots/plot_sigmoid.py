...
from safe_activations.approx import approx_sigmoid
from safe_activations.exact import exact_sigmoid
...
def plot_curve_figure(
    ...
    plt.plot(x.cpu().numpy(), exact_y.cpu().numpy(), label="Exact Sigmoid", linewidth=2)
    ...
    plt.title("Sigmoid: Exact vs Polynomial Approximation")
    plt.xlabel("x")
    plt.ylabel("y")
...
def plot_error_figure(
    ...
    plt.title("Sigmoid Approximation Absolute Error")
    plt.xlabel("x")
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot Sigmoid approximation curves and errors.")
...
def main() -> None:
    ...
    exact_y = exact_sigmoid(x)

    approx_results: Dict[int, torch.Tensor] = {}
    metrics: List[dict] = []

    for degree in degrees:
        approx_y = approx_sigmoid(
            x,
            degree=degree,
            interval=interval,
            method=args.method,
    ...
    curve_path = figure_dir / "sigmoid_curves.png"
    error_path = figure_dir / "sigmoid_errors.png"
    metrics_path = table_dir / "sigmoid_metrics.json"
    ...
    print("Sigmoid plotting finished.")
