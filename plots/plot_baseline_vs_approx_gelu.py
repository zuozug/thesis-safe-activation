...
def plot_comparison(baseline: Dict[str, Any], approx_gelu: Dict[str, Any], output_path: Path) -> None:
    fig, axes = plt.subplots(2, 2, figsize=(12, 9))

    # 1) 训练损失曲线
    ax = axes[0, 0]
    ax.plot(baseline["epochs"], baseline["train_loss"], marker="o", linewidth=2, label=baseline["label"])
    ax.plot(approx_gelu["epochs"], approx_gelu["train_loss"], marker="o", linewidth=2, label=approx_gelu["label"])
    ...
    ax.plot(approx_gelu["epochs"], approx_gelu["val_accuracy"], marker="o", linewidth=2, label=approx_gelu["label"])
    ...
    # 3) 最终测试准确率柱状图
    ax = axes[1, 0]
    model_names = [baseline["label"], approx_gelu["label"]]
    test_acc = [baseline["final_test_accuracy"], approx_gelu["final_test_accuracy"]]
    ...
    # 4) 总训练时间柱状图
    ax = axes[1, 1]
    train_times = [baseline["total_training_seconds"], approx_gelu["total_training_seconds"]]
    ...
    fig.suptitle("Baseline vs ApproxGELU Comparison", fontsize=14)
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()


def save_summary(baseline: Dict[str, Any], approx_gelu: Dict[str, Any], output_path: Path) -> None:
    summary = {
        "baseline": {
        ...
        "approx_gelu": {
            "final_test_accuracy": approx_gelu["final_test_accuracy"],
            "total_training_seconds": approx_gelu["total_training_seconds"],
        },
        "delta": {
            "test_accuracy_diff": approx_gelu["final_test_accuracy"] - baseline["final_test_accuracy"],
            "training_time_diff_seconds": approx_gelu["total_training_seconds"] - baseline["total_training_seconds"],
        },
    }
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plot Baseline vs ApproxGELU comparison figure."
    )
    parser.add_argument(
        "--baseline",
        type=str,
...
def main() -> None:
    ...
    approx_gelu_path = resolve_path(args.approx_gelu)
    ...
    approx_gelu_metrics = load_json(approx_gelu_path)

    baseline = extract_core_info(baseline_metrics, "Baseline")
    approx_gelu = extract_core_info(approx_gelu_metrics, "ApproxGELU")

    figure_path = figure_dir / "baseline_vs_approx_gelu_comparison.png"
    summary_path = table_dir / "baseline_vs_approx_gelu_summary.json"

    plot_comparison(baseline, approx_gelu, figure_path)
    save_summary(baseline, approx_gelu, summary_path)
