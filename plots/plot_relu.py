...
from safe_activations.approx import approx_relu
from safe_activations.exact import exact_relu
...
def plot_curve_figure(
    ...
    plt.plot(x.cpu().numpy(), exact_y.cpu().numpy(), label="Exact ReLU", linewidth=2)
    ...
    plt.title("ReLU: Exact vs Polynomial Approximation")
    plt.xlabel("x")
    plt.ylabel("y")
...
def plot_error_figure(
    ...
    plt.title("ReLU Approximation Absolute Error")
    plt.xlabel("x")
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot ReLU approximation curves and errors.")
...
def main() -> None:
    ...
    exact_y = exact_relu(x)

    approx_results: Dict[int, torch.Tensor] = {}
    metrics: List[dict] = []

    for degree in degrees:
        approx_y = approx_relu(
            x,
            degree=degree,
            interval=interval,
            method=args.method,
    ...
    curve_path = figure_dir / "relu_curves.png"
    error_path = figure_dir / "relu_errors.png"
    metrics_path = table_dir / "relu_metrics.json"
